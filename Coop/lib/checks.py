"""Independent CloudWatch Logs Insights / AWS API health-check functions for Coop.
...
def _run_stream(logs_client, log_group: str, stream: str, lookback_minutes: int,
    ...
    """
    # Use a quoted substring match, not /regex/ - Coop's stream names contain literal "/"
...
def check_factiva_import(logs_client, log_group: str, lookback_minutes: int) -> dict:
    """norkom.log: Factiva PFA file processing and FPFA list import-completion activity."""
    rows = _run_stream(
...
def check_payment_fraud(logs_client, log_group: str, lookback_minutes: int) -> dict:
    """norkom.log: confirm the payment fraud batch run completed."""
    rows = _run_stream(
        logs_client, log_group, config.LOG_STREAMS["norkom"], lookback_minutes,
        limit=20, message_filter="@message like /Processing Complete/",
        fallback=True,
    )
    if not rows:
        return {"status": FAILED, "detail": "No 'Processing Complete' activity found in norkom.log"}
