"""Independent CloudWatch Logs Insights / AWS API health-check functions for TabCorp.
...
def _run_stream(logs_client, log_group: str, stream: str, lookback_minutes: int,
    ...
    """
    # Use a quoted substring match, not /regex/ - TabCorp's stream names contain literal "/"
...
def check_worldcheck_import(logs_client, log_group: str, lookback_minutes: int) -> dict:
    """application.log: World-Check list import start and completion activity."""
    rows = _run_stream(
    ...
    if not rows:
        return {"status": FAILED, "detail": "No World-Check import activity found in application.log"}
    ...
    completed = any("end of worldcheck list import" in m.lower() for m in messages)
    status = FAILED if has_failure else HEALTHY if completed else WARNING
    plural = "y" if len(rows) == 1 else "ies"
    detail = f"{len(rows)} World-Check import entr{plural} - latest: {messages[0][:200]}"
...
def check_acq_failure_flag(logs_client, log_group: str, lookback_minutes: int) -> dict:
    """runBatch.log: verify no acq_fail flag was created (presence of this flag IS the failure signal, unlike acq_success)."""
    rows = _run_stream(
        logs_client, log_group, config.LOG_STREAMS["run_batch"], lookback_minutes,
        limit=5, message_filter="@message like /acq_fail /",
    )
    if not rows:
        return {"status": HEALTHY, "detail": "No 'acq_fail' flag activity found in runBatch.log"}

    latest = rows[0].get("@message", "")
    match = re.search(r"(?i)(acq_fail\S*\.flag)", latest)
    flag_name = match.group(1) if match else latest[:150]
