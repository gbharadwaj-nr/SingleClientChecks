"""Independent CloudWatch Logs Insights / AWS API health-check functions for LFS.
...
def _run_stream(logs_client, log_group: str, stream: str, lookback_minutes: int,
    ...
    """
    # Use a quoted substring match, not /regex/ - LFS's stream names contain literal "/"
...
def check_rds_maintenance(session, all_regions: list[str], lookback_minutes: int) -> dict:
    ...
    account 616476889381 is shared with other clients (IAG/MGL/Suncorp), so results are
